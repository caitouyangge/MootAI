...
public class AiService {
    ...
    private String buildInstruction(String currentRole, String judgeType, String userIdentity, String opponentStrategy) {
        ...
        if ("judge".equalsIgnoreCase(currentRole)) {
            ...
            instruction.append("7. 【重要】法庭辩论的发言顺序：原告先发言，然后被告发言，每完成一轮（原告+被告）后，你判断是否需要介入。只有在以下情况才需要介入：\n");
            ...
            instruction.append("8. 【重要】非必要不介入，不说废话。如果双方辩论正常进行，没有程序问题，没有需要纠正的地方，就不要发言。让原告和被告继续轮流发言即可。\n");
            instruction.append("9. 【重要】如果你决定发言，发言内容必须简洁、专业、有针对性，不要说套话、空话。发言后必须明确指定下一个发言人的身份（\"请原告继续\"或\"请被告继续\"）。\n");
            instruction.append("10. 【重要】如果不需要发言，只输出\"不需要发言\"，然后由原告和被告继续轮流发言。\n");
            instruction.append("11. 【重要】法庭辩论中只有\"审判员\"、\"原告\"、\"被告\"三个角色可以发言。案件背景中可能包含各种实体名称（如机构名称、公司名称等），但这些不是法庭角色，不能作为发言人的身份。当你指定下一个发言人时，必须明确使用\"请原告继续\"或\"请被告继续\"，绝对不能使用案件背景中的机构名称、公司名称等作为发言人身份。\n");
            instruction.append("12. 【重要】作为审判员，你绝对不能让自己发言，不能说\"请审判员发言\"、\"请审判员继续\"等类似的话。你只能指定\"原告\"或\"被告\"作为下一个发言人。\n");
            instruction.append("13. 【重要】当辩论阶段结束时，你必须发表完整的结束语，不能只说\"辩论阶段结束\"。结束语应包含：对双方辩论内容的总结、对争议焦点的归纳、对从宽从重情节的说明、对案件复杂性的认识、以及表明将依法公正判决的态度。示例格式：\"双方围绕[争议焦点]等问题已进行多轮充分辩论。法庭注意到，[从宽情节]，但[从重情节]的事实清楚，社会危害性[评价]。如何在依法惩处与合理考量其特殊情况之间作出裁量，本庭将严格依照法律规定，结合全案事实与证据，作出公正判决。辩论阶段结束。\"");
