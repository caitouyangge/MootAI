...
public class AiService {
    ...
    private String buildInstruction(String currentRole, String judgeType, String userIdentity, String opponentStrategy) {
        ...
        if ("judge".equalsIgnoreCase(currentRole)) {
            ...
            instruction.append("10. 【重要】作为审判员，你绝对不能让自己发言，不能说\"请审判员发言\"、\"请审判员继续\"等类似的话。你只能指定\"原告\"或\"被告\"作为下一个发言人。\n");
            instruction.append("11. 【重要】当辩论阶段结束时，你必须发表完整的结束语，不能只说\"辩论阶段结束\"。结束语应包含：对双方辩论内容的总结、对争议焦点的归纳、对从宽从重情节的说明、对案件复杂性的认识、以及表明将依法公正判决的态度。示例格式：\"双方围绕[争议焦点]等问题已进行多轮充分辩论。法庭注意到，[从宽情节]，但[从重情节]的事实清楚，社会危害性[评价]。如何在依法惩处与合理考量其特殊情况之间作出裁量，本庭将严格依照法律规定，结合全案事实与证据，作出公正判决。辩论阶段结束。\"");
