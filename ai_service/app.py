...
def build_system_prompt_from_training_format(agent_role, background, instruction):
    ...
    if agent_role == '审判员':
        ...
        base_prompt += "5. 【重要】作为审判员，你绝对不能让自己发言，不能说\"请审判员发言\"、\"请审判员继续\"等类似的话。\n"
        base_prompt += "   你只能指定\"原告\"或\"被告\"作为下一个发言人。\n"
        base_prompt += "6. 【重要】当辩论阶段结束时，你必须发表完整的结束语，不能只说\"辩论阶段结束\"。\n"
...
def build_system_prompt(user_identity, current_role, judge_type, case_description):
    ...
    if current_role == 'judge':
        ...
        base_prompt += "2. 当你需要指定下一个发言人时，只能使用\"请原告继续\"或\"请被告继续\"。\n"
        base_prompt += "3. 法庭辩论中只有\"审判员\"、\"原告\"、\"被告\"三个角色可以发言，案件背景中的机构名称、公司名称等不是法庭角色。\n"
        base_prompt += "4. 【重要】当辩论阶段结束时，你必须发表完整的结束语，不能只说\"辩论阶段结束\"。\n"
        base_prompt += "   结束语应包含：对双方辩论内容的总结、对争议焦点的归纳、对从宽从重情节的说明、\n"
        base_prompt += "   对案件复杂性的认识、以及表明将依法公正判决的态度。\n"
        base_prompt += "   示例格式：\"双方围绕[争议焦点]等问题已进行多轮充分辩论。法庭注意到，[从宽情节]，\n"
        base_prompt += "   但[从重情节]的事实清楚，社会危害性[评价]。如何在依法惩处与合理考量其特殊情况之间作出裁量，\n"
