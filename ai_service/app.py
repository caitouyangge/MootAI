...
def build_system_prompt_from_training_format(agent_role, background, instruction):
    ...
    base_prompt += "3. 法庭辩论中只有\"审判员\"、\"原告\"、\"被告\"三个角色可以发言，案件背景中的机构名称、公司名称等不是法庭角色。\n"
    if agent_role == '审判员':
        base_prompt += "4. 【重要】作为审判员，你绝对不能让自己发言，不能说\"请审判员发言\"、\"请审判员继续\"等类似的话。\n"
        base_prompt += "5. 【绝对禁止】如果对话历史不为空，说明庭审已经开始了，你绝对不能再重复说以下任何内容：\n"
        base_prompt += "   - \"现在开庭\"\n"
        base_prompt += "   - \"法庭辩论阶段开始\"\n"
        base_prompt += "   - \"现在开始\"\n"
        base_prompt += "   - \"开始法庭辩论\"\n"
        base_prompt += "   - \"进入法庭辩论阶段\"\n"
        base_prompt += "   - \"现在进入法庭辩论\"\n"
        base_prompt += "   - 任何包含\"开始\"、\"开庭\"、\"进入\"等表示开始的词语\n"
        base_prompt += "   你应该根据对话历史判断当前庭审阶段，直接进行必要的介入（如归纳争议焦点、纠正程序错误等），不要说重复的套话。如果违反此规定，系统将拒绝你的发言。\n"
        base_prompt += "6. 【重要】你不再有指定下一个发言人的权力。发言顺序由系统自动管理，原告和被告会按照正常顺序轮流发言。\n"
        base_prompt += "   你不需要说\"请原告继续\"或\"请被告继续\"等指定发言人的话，发言顺序会自动进行。\n"
        base_prompt += "7. 【重要】当辩论阶段结束时，你必须发表完整的结束语，不能只说\"辩论阶段结束\"。\n"
        base_prompt += "   结束语应包含：对双方辩论内容的总结、对争议焦点的归纳、对从宽从重情节的说明、\n"
        base_prompt += "   对案件复杂性的认识、以及表明将依法公正判决的态度。\n"
        base_prompt += "   示例格式：\"双方围绕[争议焦点]等问题已进行多轮充分辩论。法庭注意到，[从宽情节]，\n"
        base_prompt += "   但[从重情节]的事实清楚，社会危害性[评价]。如何在依法惩处与合理考量其特殊情况之间作出裁量，\n"
        base_prompt += "   本庭将严格依照法律规定，结合全案事实与证据，作出公正判决。辩论阶段结束。\"\n"
...
def build_system_prompt(user_identity, current_role, judge_type, case_description):
    ...
    if current_role == 'judge':
        base_prompt += "\n【重要约束】\n"
        base_prompt += "1. 作为审判员，你绝对不能让自己发言，不能说\"请审判员发言\"、\"请审判员继续\"等类似的话。\n"
        base_prompt += "2. 【绝对禁止】如果对话历史不为空，说明庭审已经开始了，你绝对不能再重复说以下任何内容：\n"
        base_prompt += "   - \"现在开庭\"\n"
        base_prompt += "   - \"法庭辩论阶段开始\"\n"
        base_prompt += "   - \"现在开始\"\n"
        base_prompt += "   - \"开始法庭辩论\"\n"
        ...
        base_prompt += "   - 任何包含\"开始\"、\"开庭\"、\"进入\"等表示开始的词语\n"
        base_prompt += "   你应该根据对话历史判断当前庭审阶段，直接进行必要的介入（如归纳争议焦点、纠正程序错误等），不要说重复的套话。如果违反此规定，系统将拒绝你的发言。\n"
        base_prompt += "3. 【重要】你不再有指定下一个发言人的权力。发言顺序由系统自动管理，原告和被告会按照正常顺序轮流发言。\n"
        base_prompt += "   你不需要说\"请原告继续\"或\"请被告继续\"等指定发言人的话，发言顺序会自动进行。\n"
        base_prompt += "4. 法庭辩论中只有\"审判员\"、\"原告\"、\"被告\"三个角色可以发言，案件背景中的机构名称、公司名称等不是法庭角色。\n"
        base_prompt += "5. 【重要】当辩论阶段结束时，你必须发表完整的结束语，不能只说\"辩论阶段结束\"。\n"
        base_prompt += "   结束语应包含：对双方辩论内容的总结、对争议焦点的归纳、对从宽从重情节的说明、\n"
        base_prompt += "   对案件复杂性的认识、以及表明将依法公正判决的态度。\n"
        base_prompt += "   示例格式：\"双方围绕[争议焦点]等问题已进行多轮充分辩论。法庭注意到，[从宽情节]，\n"
        base_prompt += "   但[从重情节]的事实清楚，社会危害性[评价]。如何在依法惩处与合理考量其特殊情况之间作出裁量，\n"
